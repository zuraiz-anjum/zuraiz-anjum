...
def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    """
    Recursively calls recursive_loc (GraphQL can only fetch 100 commits at a time),
...
def loc_query(owner_affiliation, comment_size=0, cursor=None, edges=None):
    """
    Queries every repository accessible to the user, 60 at a time, then hands off to cache_builder.
    """
    if edges is None:
        edges = []
    ...
    if repos['pageInfo']['hasNextPage']:
        return loc_query(owner_affiliation, comment_size, repos['pageInfo']['endCursor'], edges + repos['edges'])
    return cache_builder(edges + repos['edges'], comment_size)


def cache_builder(edges, comment_size, loc_add=0, loc_del=0):
    """
    Checks each repo against the local cache; recomputes LOC only for repos whose commit count changed.
    """
...
def flush_cache(edges, filename, comment_size):
    """
    Wipes and rebuilds the cache file when the tracked repo count changes.
    """
    with open(filename, 'r') as f:
...
def force_close_file(data, cache_comment):
    ...
    with open(filename, 'w') as f:
        f.writelines(cache_comment)
        f.writelines(data)
    print('There was an error while writing to the cache file. Partial data has been saved to', filename)
...
def svg_overwrite(filename, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    """
    Parses the SVG and overwrites the id-tagged stat fields with live values.
    """
...
def justify_format(root, element_id, new_text, length=0):
    """
    Updates the text of an id-tagged element and pads the preceding dot-leader to match.
    """
...
def commit_counter(comment_size):
    """
    Sums commit counts recorded in the local cache file.
    """
    total_commits = 0
    filename = 'cache/' + hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest() + '.txt'
    with open(filename, 'r') as f:
...
def user_getter(username):
    """
    Returns the account's GraphQL node id.
    """
...
if __name__ == '__main__':
    """
    Zuraiz Anjum profile card generator.
    """
    ...
    svg_overwrite('dark.svg', commit_data, star_data, repo_data, contrib_data, follower_data, total_loc)
    svg_overwrite('light.svg', commit_data, star_data, repo_data, contrib_data, follower_data, total_loc)
